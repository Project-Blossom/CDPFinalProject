...
EBTNodeResult::Type UBTTask_ChargeAttack::ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
    ...
    if (!TargetPlayer)
    {
        UE_LOG(LogTemp, Error, TEXT("BTTask_ChargeAttack: No target player"));
        return EBTNodeResult::Failed;
    }
    ...
    UE_LOG(LogTemp, Log, TEXT("BTTask_ChargeAttack: Charging attack toward player"));
...
void UBTTask_ChargeAttack::TickTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds)
{
    ...
    {
        UE_LOG(LogTemp, Log, TEXT("BTTask_ChargeAttack: Charge complete"));
